...
std::pair<int, int> get_new_img_size(int height, int width, InputParam& input){
    ...
    if (width <= height) {
        input.isWidthOverHeight = false;
        f = static_cast<double>(input.modelLenth) / width;
        resized_height = static_cast<int>(f * height);
        resized_width = input.modelLenth;
    } else {
        input.isWidthOverHeight = true;
        f = static_cast<double>(input.modelLenth) / height;
        resized_width = static_cast<int>(f * width);
        resized_height = input.modelLenth;
    }

    return std::make_pair(resized_height, resized_width);
}
...
cv::Mat resize_image(const cv::Mat& image, InputParam& input){
    ...
    // 使用双三次插值算法进行缩放
    cv::Mat new_image;
    cv::resize(image, new_image, cv::Size(w, h), 0, 0, cv::INTER_CUBIC);
